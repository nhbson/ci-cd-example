...
class App(tk.Tk):
    ...
    def setup_editor_tab(self):
        ...
        left_col = ttk.Frame(main, width=350); left_col.pack(side="left", fill="y", padx=5)
        glob = ttk.LabelFrame(left_col, text="Global Config"); glob.pack(fill="x", pady=(0, 10))
        ...
        list_frame = ttk.LabelFrame(left_col, text="Saved Tasks"); list_frame.pack(fill="both", expand=True)
        self.ui_task_list = tk.Listbox(list_frame, font=("Segoe UI", 10), bd=0, highlightthickness=0); self.ui_task_list.pack(fill="both", expand=True, padx=5, pady=5)
        ...
        right_col = ttk.Frame(main); right_col.pack(side="right", fill="both", expand=True, padx=5)
        edit_scroll = tk.Canvas(right_col, highlightthickness=0); scrollbar = ttk.Scrollbar(right_col, orient="vertical", command=edit_scroll.yview)
        self.scrollable_frame = ttk.Frame(edit_scroll); self.scrollable_frame.bind("<Configure>", lambda e: edit_scroll.configure(scrollregion=edit_scroll.bbox("all")))
        edit_scroll.create_window((0, 0), window=self.scrollable_frame, anchor="nw"); edit_scroll.configure(yscrollcommand=scrollbar.set)
        edit_scroll.pack(side="left", fill="both", expand=True); scrollbar.pack(side="right", fill="y")
        ...
        pg = ttk.LabelFrame(self.scrollable_frame, text="Pagination Logic"); pg.pack(fill="x", pady=10)
        ...
        gs = ttk.LabelFrame(self.scrollable_frame, text="Google Sheets Target"); gs.pack(fill="x", pady=10)
        self.ui_sid = self.field(gs, "Sheet ID"); self.ui_tab = self.field(gs, "Tab Name")
        sel_frame = ttk.LabelFrame(self.scrollable_frame, text="Data Extraction (Column Name : CSS Selector)"); sel_frame.pack(fill="x", pady=10)
        f_input = ttk.Frame(sel_frame); f_input.pack(fill="x", padx=5, pady=5)
        ...
        f_btns = ttk.Frame(sel_frame); f_btns.pack(fill="x", padx=5)
        ...
        self.ui_fields_box = tk.Listbox(sel_frame, height=5); self.ui_fields_box.pack(fill="x", padx=5, pady=5)
    ...
    def create_label_entry(self, parent, txt, default, browse=False):
        frame = ttk.Frame(parent); frame.pack(fill="x", padx=5, pady=2)
    ...
    def sync_from_json_text(self):
        try:
            self.tasks = json.loads(self.json_editor.get("1.0", tk.END)); self.save_tasks_to_disk()
            messagebox.showinfo("Success", "Updated from JSON")
    ...
    # ================= CORE ENGINE (UPDATED PAGINATION) =================
    def engine_start(self, task_list):
        ...
        except Exception as e: self.log(f"❌ Global Error: {e}")
    ...
    def process_task(self, task, gc):
        ...
        except: self.log("❌ Sheet Access Failed"); return

        try:
            session.get("https://doda.jp/", timeout=20)
            res = session.get(task['url'], timeout=30)
            soup = BeautifulSoup(res.text, "html.parser")
        except Exception as e: self.log(f"❌ Access Blocked: {e}"); return

        # FIX FOR LARGE NUMBERS (2,371)
        total_items = 0
        ...
        self.log(f"📊 Items Found: {total_items} | Total Pages: {total_pages}")
        ...
        for p in range(1, total_pages + 1):
            ...
            with ThreadPoolExecutor(max_workers=2) as ex:
                ...
                for f in as_completed(futures):
                    ...
                    if r:
                        results.append(r)
                        self.log(f"   ✅ [{count}/{len(links)}] Extracted: {str(r.get('会社名', 'N/A'))[:15]}...")
                    else: self.log(f"   ❌ Failed Job Page")
