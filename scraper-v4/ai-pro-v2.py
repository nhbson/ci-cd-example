...
import gspread
# REPLACED REQUESTS WITH CURL_CFFI FOR BYPASS
...
class App(tk.Tk):
    ...
    def field(self, parent, label):
        ...
        e = tk.Entry(frame, font=("Segoe UI", 10)); e.pack(fill="x", pady=2)
        return e
    ...
    def clear_task_form(self):
        ...
        for e in [self.ui_name, self.ui_url, self.ui_link_sel, self.ui_stats_sel, self.ui_page_key, self.ui_sid, self.ui_tab]: e.delete(0, tk.END)
        self.dynamic_fields = {}; self.refresh_fields_list(); self.status_var.set("New Mode")
    ...
    def run(self, mode="all"):
        ...
        tasks_to_run = [self.tasks[self.selected_task_index]] if mode == "selected" else self.tasks
    ...
    # ================= CORE ENGINE (DODA LARGE NUMBER FIX) =================
    ...
    def process_task(self, task, gc):
        ...
        # CORRECT MATH FOR LARGE NUMBERS (e.g., 2,371)
        total_items = 0
        ...
        for p in range(1, total_pages + 1):
            ...
            if not links:
                self.log("⚠️ No jobs found. Possible block."); break
            ...
            if results:
                ...
                self.log(f"💾 Page {p} saved to Sheet.")
    ...
    def fetch_detail(self, session, url, fields):
        ...
        try:
            ...
            for k, v in fields.items():
                ...
                for selector in v.split(','):
                    el = s.select_one(selector.strip())
                    if el:
